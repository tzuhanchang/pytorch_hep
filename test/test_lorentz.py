...
def test_MomentumTensor():
    ...
    assert torch.all(torch.isclose(long.select(-1,0),long_EEtaPhiPt.e))
    assert torch.all(torch.isclose(short[1],short_EEtaPhiPt.px))
    assert torch.all(torch.isclose(long.select(-1,1),long_EEtaPhiPt.px))
    assert torch.all(torch.isclose(short[2],short_EEtaPhiPt.py))
    assert torch.all(torch.isclose(long.select(-1,2),long_EEtaPhiPt.py))
    assert torch.all(torch.isclose(short[3],short_EEtaPhiPt.pz))
    assert torch.all(torch.isclose(long.select(-1,3),long_EEtaPhiPt.pz))

    assert torch.all(torch.isclose(short[0],short_MEtaPhiPt.e))
    assert torch.all(torch.isclose(long.select(-1,0),long_MEtaPhiPt.e))
    assert torch.all(torch.isclose(short[1],short_MEtaPhiPt.px))
    assert torch.all(torch.isclose(long.select(-1,1),long_MEtaPhiPt.px))
    assert torch.all(torch.isclose(short[2],short_MEtaPhiPt.py))
    assert torch.all(torch.isclose(long.select(-1,2),long_MEtaPhiPt.py))
    assert torch.all(torch.isclose(short[3],short_MEtaPhiPt.pz))
    assert torch.all(torch.isclose(long.select(-1,3),long_MEtaPhiPt.pz))
